...
export default function Month() {
  ...
  const count = useCallback(() => {
    const { finalCut, moneyOfROZP, myBonus, myPureMoney, coefficientString, KPIAfterTaxes } = calculateSalary(
      cases,
      user?.userOklad,
    );
    ...
    dispatch(updateMoney({ rozpMoney: moneyOfROZP, myTake: myBonus, myPureMoney }));
    ...
    <>
      ...
      <ToastContainer position="bottom-left"/>
        <div className='month'>
          <h5>{formattedDate}</h5>
          {cases.length >= 1 && (
            <button className='greenBtn' onClick={() => dispatch(changeDialog('show'))}>
          ...
          <Link to='trashbin'>
            <span className='material-symbols-outlined green-trash-bin'>delete</span>
          </Link>
          <Link to='sessions'>
            <span className='material-symbols-outlined green-trash-bin'>gavel</span>
          </Link>
        </div>
        ...
        {error && (
          <div className='containder-for-loader'>
            <h1>
              На сервере ошибка, которая не позволяет использовать данный сервис: {error}
            </h1>
          </div>
        )}
            ...
            <tr>
              {cases.length > 1 && status !== 'loading' ? (
                <th>
                  №{' '}
                  {isDown5 ? (
                    <span className='material-symbols-outlined' onClick={() => sortTable('isPaid')}>
                      arrow_upward
                    </span>
                  ) : (
                    <span className='material-symbols-outlined' onClick={() => sortTable('isPaid')}>
              ...
              <th>
                Ответчик:{' '}
                {isDown1 ? (
                  <span className='material-symbols-outlined' onClick={() => sortTable('name')}>
                    arrow_upward
                  </span>
                ) : (
                  <span className='material-symbols-outlined' onClick={() => sortTable('name')}>
              ...
              <th>
                Расходы РОЗП:{' '}
                {isDown2 ? (
                  <span className='material-symbols-outlined' onClick={() => sortTable('expenses')}>
                    arrow_upward
                  </span>
                ) : (
                  <span className='material-symbols-outlined' onClick={() => sortTable('expenses')}>
              ...
              <th>
                Доход до налогов:{' '}
                {isDown3 ? (
                  <span className='material-symbols-outlined' onClick={() => sortTable('thirty')}>
                    arrow_upward
                  </span>
                ) : (
                  <span className='material-symbols-outlined' onClick={() => sortTable('thirty')}>
              ...
              <th>
                Чистый доход:{' '}
                {isDown4 ? (
                  <span className='material-symbols-outlined' onClick={() => sortTable('bonus')}>
                    arrow_upward
                  </span>
                ) : (
                  <span className='material-symbols-outlined' onClick={() => sortTable('bonus')}>
              ...
              <th>
                Доли:{' '}
                {isDown6 ? (
                  <span className='material-symbols-outlined' onClick={() => sortTable('takes')}>
                    arrow_upward
                  </span>
                ) : (
                  <span className='material-symbols-outlined' onClick={() => sortTable('takes')}>
              ...
              <th>
                Мои доли:{' '}
                {isDown7 ? (
                  <span className='material-symbols-outlined' onClick={() => sortTable('myTakes')}>
                    arrow_upward
                  </span>
                ) : (
                  <span className='material-symbols-outlined' onClick={() => sortTable('myTakes')}>
        ...
        <div className='result'>
          ...
          <h1>
            Мои чистые деньги: <span className='money'>{myPureMoney}</span> бел. руб.
          </h1>
          <h1>
