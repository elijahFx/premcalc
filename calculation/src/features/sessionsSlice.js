...
export const sessionsSlice = createSlice({
  ...
  extraReducers: {
    ...
    [deleteConsumer.fulfilled]: (state, action) => {
      state.status = "resolved";
      state.consumerList = state.consumerList.filter(
    ...
    [deleteSession.fulfilled]: (state, action) => {
      state.status = "resolved";
      state.courtSessions = state.courtSessions.filter(
        (el) => el._id !== action?.meta?.arg
      );
    },
    ...
    [checkCourtSessionsForConsumers.fulfilled]: (state, action) => {
      ...
      if (arrayLength === 0) {
        ...
        toast.success(
          `${addedSessions[0].name} к ${addedSessions[0].liabelee} успешно добавлено`
        );
      } else if (arrayLength >= 2) {
        state.courtSessions = [...state.courtSessions, ...addedSessions];
        const info = [...addedSessions]
          .map((el, index) => {
...
export const { addTheConsumer, removeConsumer, removeSession } =
