  ...
  async function (auth, { rejectWithValue, dispatch }) {
    try {
      const response = await fetch(
        `${BASIC_URL}users/reset-password/${auth.id}/${auth.token}`,
        {}
      );
...
export const getStatistics = createAsyncThunk(
  ...
  async function (id, { rejectWithValue, dispatch }) {
    try {
      const response = await fetch(`${BASIC_URL}users/getstatistics`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
...
export const { login, signup, logout, getToken, getAllUsers, updateUser } =
  usersSlice.actions;
