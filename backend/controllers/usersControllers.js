...
async function editUser(req, res) {
  ...
  try {
    ...
    if (!image && !name && !userOklad) {
      return res
        .status(400)
        .json({ err: "Пожалуйста, укажите имя или изображение" });
    }
  } catch (error) {
    console.log(error);
...
async function forgotPassword(req, res) {
  ...
  try {
    ...
    const mailOptions = {
      ...
      <p>Добрый день${
        isName ? `, ${oldUser.name}` : ""
      }. Нам поступила информация о том, что Вам необходим новый пароль. Для того, чтобы установить новый пароль для Вашего аккаунта, перейдите по следующей ссылке - <a href="${link}">${link}</a>.</p>
...
async function addStatistic(req, res) {
  try {
    ...
    for (let user of users) {
      ...
      const cases = await Case.find({ user_id: user._id }).sort({
        createdAt: 1,
      });
      ...
      const isStatisticExists = statistics.some(
    ...
    return res
      .status(200)
      .json({ message: "Статистика обновлена для всех пользователей" });
  } catch (error) {
    console.error(error);
